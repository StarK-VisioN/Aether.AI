  ...
  return user ? (
    <div className='flex flex-col items-start justify-start h-screen'>
        <nav className='w-full px-8 min-h-14 flex items-center justify-between border-b border-gray-400'>
          ...
          {
            sidebar ? <X onClick={()=>setSidebar(false)} className='w-6 h-6 text-gray-600 sm:hidden' />     // hidden on small screen
        ...
        <div className='flex-1 w-full flex h-[calc(100vh-64px)]'>
          ...
          <div className='flex-1 bg-[#F4F7FB]'>
            <Outlet />       {/* Layout routing childs will render here e.g. :- /ai/write-article    & rendering of common section i.e. Navbar & sidebar for available options */}
          </div>
        </div>
